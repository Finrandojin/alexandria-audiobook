...
ignored by omission.
"""
from audio_validation import (GeneratedAudioError, remove_stale_audio,
                              validate_generated_audio)


class GenerationFailed(GeneratedAudioError):
    """Compatibility name for an experiment render failure."""
...
def render(engine, text, instruct, speaker, voice_config, voice_data, path):
    ...
    try:
        validate_generated_audio(
            path, f"{category} generation for {speaker!r}")
...
def _check_riff_completeness(path, category="", speaker=""):
    """Compatibility wrapper for callers that used the old private check."""
    try:
        validate_generated_audio(
            path, f"{category} generation for {speaker!r}")
    except GeneratedAudioError as exc:
